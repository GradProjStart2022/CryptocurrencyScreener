...
class Upbit_Gathering():
    ...
    def gathering(self, end_date=''):
        ...
        for i in tq_range:
            ...
            try:
                local_sql = f'''select date, open, high, low, close, volume, RSI from KRW_{name} order by date desc limit 10;'''
                            ...
                            if self.interval == '1w' or self.interval == '1M':
                                if row['date'] in local_db_df['date'].to_list():
                                    ...
                                    insert_sql = f"""
                                                ...
                                                values ('{row['date']}','{row['open']}','{row['high']}','{row['low']}','{row['close']}','{row['volume']}', RSI='{row['RSI']}'
                                                """
